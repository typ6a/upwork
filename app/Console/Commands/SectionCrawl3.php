...
class SectionCrawl3 extends Command
{
   
    protected $signature = 'crawl:section3';
    ...
    protected $base_url1 = 'http://www.gosection8.com/Tenant/tn_Results.aspx?Address=Riverside%20County,%20Ca&minRent=0&maxRent=5000&propertyTypeList=All%20Property%20Type&bedrooms=0&bathrooms=1&Accessible=False&radius=&pictures=False&pets=False&ac=False&coveredParking=False&MaxSqFt=5000&MinSqFt=0&keyword=&AgeRestricted=False&smoking=False&pg=';
    protected $pagesNum1 = 19;
    ...
    protected function getPagesUrlByPhoto(){
        ...
        for ($pageNum = 0; $pageNum <= $this->pagesNum1; $pageNum++){
            ...
            $pageHtmlPath = storage_path('app/section/3_' . $pageNum . '.html');
                //pre($pageHtmlPath,1);
    ...
    protected function getPropertiesDetails(){
        ...
        $fh = fopen(storage_path('app/section/section8_3.csv'), 'w');
        $head = [
            'Name',
            'Phone',
            'Address',
        ...
        fputcsv($fh, $head, ';');
        foreach ($urls as $url) {
        $propertyHtmlPath = storage_path('app/section/3_' . md5($url) . '.html');
