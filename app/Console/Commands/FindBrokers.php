...
class FindBrokers extends Command
{
     
    protected $signature = 'find:brokers';
    ...
    protected $base_url = 'http://www.gosection8.com/Tenant/tn_Results.aspx?Address=San+Bernardino+County%2c+Ca&minRent=0&maxRent=5000&propertyTypeList=House|Townhouse/Villa|&bedrooms=0&bathrooms=1&Accessible=False&pictures=False&pets=False&ac=False&AgeRestricted=False&smoking=False&coveredParking=False&MaxSqFt=5000&MinSqFt=0&keyword=&sortBy=LastUpdate&pg=';
    protected $pagesNum1 = 4;
    ...
    protected function getPagesUrlByPhoto(){
        ...
        for ($pageNum = 0; $pageNum <= $this->pagesNum1; $pageNum++){
            $pageUrl = $this->base_url . $pageNum;
            $pageHtmlPath = storage_path('app/section/4_' . $pageNum . '.html');
                //pre($pageHtmlPath,1);
    ...
    protected function getPropertiesDetails(){
        ...
        $fh = fopen(storage_path('app/section/section8_4.csv'), 'w');
        $head = [
            'Name',
            'Phone',
            'Address',
        ...
        fputcsv($fh, $head, ';');
        foreach ($urls as $url) {
        $propertyHtmlPath = storage_path('app/section/4_' . md5($url) . '.html');
