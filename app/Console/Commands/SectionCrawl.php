...
class SectionCrawl extends Command
{
   
    protected $signature = 'crawl:section';
    ...
    protected $base_url1 = 'http://www.gosection8.com/Tenant/tn_Results.aspx?Address=Los+Angeles+County%2c+Ca&minRent=0&maxRent=5000&propertyTypeList=House|Townhouse/Villa|&bedrooms=0&bathrooms=1&Accessible=False&pictures=False&pets=False&ac=False&AgeRestricted=False&smoking=False&coveredParking=False&MaxSqFt=5000&MinSqFt=0&keyword=&sortBy=LastUpdate&pg=';
    protected $pagesNum1 = 11;
    ...
    protected function getPagesUrlByPhoto(){
            ...
            $JsonPath = storage_path('app/section/' . 'urls.json');
            $urls = [];
        for ($pageNum = 0; $pageNum <= $this->pagesNum1; $pageNum++){
            $pageUrl = $this->base_url1 . $pageNum;
            $pageHtmlPath = storage_path('app/section/1_' . $pageNum . '.html');
                //pre($pageHtmlPath,1);
    ...
    protected function getPropertiesDetails(){
        ...
        $fh = fopen(storage_path('app/section/section8_1.csv'), 'w');
        $head = [
            'Name',
            'Phone',
            'Address',
        ...
        fputcsv($fh, $head, ';');
        foreach ($urls as $url) {
        $propertyHtmlPath = storage_path('app/section/1_' . md5($url) . '.html');
